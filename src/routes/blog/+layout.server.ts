...
export const load = (async ({ route: { id }, params: { slug } }) => {
  if (id === "/blog") {
    ...
    const { id, description, title, body: markdown, body_html, updated_at } = await getPost(slug!);

    const $ = loadCheerio(parseDocument(body_html));
    const fullText = $("div.lake-content").text();

    return { article: { id, description: fullText.startsWith(description.replace(/\.\.\.$/, "")) ? undefined : description, title, markdown, updated_at } };
  }
